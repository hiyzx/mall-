package org.zero.mall.pms.config;
...
public class Swagger2 {
    ...
    public Docket createRestApi() {
        ...
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("org.zero.mall.pms.controller")).paths(PathSelectors.any())
                .build().globalOperationParameters(parameters);
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("商城系统").description("商品服务").license("仅供内部参考")
                .contact(new Contact("zero", "https://github.com/hiyzx/mall", "452002276@qq.com")).termsOfServiceUrl(null).build();
    }
}